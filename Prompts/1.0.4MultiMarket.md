...
## Version 1.1 — Created 2026-04-04, Updated 2026-04-18
...
6. 🎾 WTA Winner (added 2026-04-18 — best ROI market)

**What's missing:** Impact information, injuries, tactical lineups, psychological momentum, match context.

---

## MARKET ROI REALITY TABLE (added 2026-04-18)

Use this to prioritize recommendations when multiple picks qualify:

| Market | Win Rate | Avg Odds | ROI/bet | Priority |
|---|---|---|---|---|
| **WTA Winner (selective 9-10/10)** | ~88% | 1.30 | **+14.4%** | **HIGHEST** |
| **DC** | 85.5% | 1.25 | **+6.9%** | **HIGH** |
| WTA Set 1 O7.5 | 83.6% | 1.22 | +0.19% | MARGINAL |
| U4.5 Goals | 86.7% | 1.15 | +0.03% | BREAK-EVEN |
| U12.5 Corners | 82.3% | 1.20 | -1.3% | NEGATIVE |

**Rules derived from ROI analysis:**
- DC is the best football market — do NOT abandon for "safer" low-odds picks
- U4.5 Goals at 1.10-1.15 is only valid in accumulators (not standalone)
- U12.5 Corners at 1.10-1.20 is NEGATIVE EV long-term — use rarely, only when mismatch < 0.3
- WTA Winner has HIGHEST potential ROI — prioritize on WTA 500+/GS events
- When multiple markets qualify, lean toward DC + WTA Winner over Goals/Corners

---

## CRITICAL LESSONS LEARNED (added 2026-04-18)

### DC Research Adjustment Cap: +3pp MAX
If stacked positive signals push research probability past +7pp vs market implied, TREAT AS RED FLAG.
Bookmakers price sharp. Large edge = trap, not opportunity.
Lesson: Udinese 1X @ 1.29 (market 77.5%) → research 88% (+10.5pp) → LOST despite "perfect setup" on 3 models.

### Corners Under: Mismatch > 0.6 = HARD PASS
When one team dominates (lambda_home >> lambda_away), corner spike pattern emerges.
Dominant team generates 8-10 corners ALONE when opponent parks bus.
Lesson: Sassuolo-Como (mismatch 0.98) + Udinese-Parma (mismatch 0.69) both hit 13+ corners despite model saying U12.5 89%+.

### Cross-model confirmation is NOT magic
A match appearing on 3 models with 9/10 felt like strong signal — but single-match variance still rules.
Use cross-model picks with same stake as single-model 9/10, not higher.
